...
int train_1layer_net(double sample[INPUTS],int label,double (*sigmoid)(double input), double weights_io[INPUTS][OUTPUTS])
{
  ...
  ***********************************************************************************************************/
  ...
  int index = classify_1layer(sample, label, sigmoid, weights_io);

  return(index);		// <--- This should return the class for this sample
}
...
void backprop_1layer(double sample[INPUTS], double activations[OUTPUTS], double (*sigmoid)(double input), int label, double weights_io[INPUTS][OUTPUTS])
{
  ...
  for (int j=0;j<OUTPUTS;j++){
    if (sigmoid == logistic){
      double final_error = target[j] - activations[j];
      errors = (activations[j] * (1- activations[j])) * final_error;
    } else {
      double final_error = target[j] - activations[j];
      errors = (1- activations[j] * activations[j]) * final_error;
    }
    for (int l=0;l<INPUTS;l++) {
      weights_io[l][j] += ALPHA * errors * sample[l];
    }
  }
...
int train_2layer_net(double sample[INPUTS],int label,double (*sigmoid)(double input), int units, double weights_ih[INPUTS][MAX_HIDDEN], double weights_ho[MAX_HIDDEN][OUTPUTS])
{
  ...
  ***********************************************************************************************************/
  ...
  int index = classify_2layer(sample, label, sigmoid,units,weights_ih, weights_ho);
  return(index);		// <--- Should return the class for this sample  
}
...
int classify_2layer(double sample[INPUTS],int label,double (*sigmoid)(double input), int units, double weights_ih[INPUTS][MAX_HIDDEN], double weights_ho[MAX_HIDDEN][OUTPUTS])
{
  ...
  ***********************************************************************************************************/
  ...
  return(max);		// <--- Should return the class for this sample  
}
  ...
  */ 
  ...
  for (int i = 0; i < units;i++) {
    ...
    for (int j = 0; j < INPUTS;j++) {
      //printf("weight is %f\n", weights_io[j][i]);
      //printf("sample is %f\n", sample[j]);
      h_sum += weights_ih[j][i] * sample[j];

    }
    //printf("sum is %f\n", sum);
    h_activations[i] = sigmoid(h_sum * SIGMOID_SCALE);
  }
  // hidden to output
  double o_sum;
  for (int i=0; i<OUTPUTS;i++){
    ...
    for (int j = 0; j < units;j++) {
      o_sum += weights_ho[j][i] * h_activations[j];
    }
    activations[i] = sigmoid(o_sum * SIGMOID_SCALE);
  }
...
void backprop_2layer(double sample[INPUTS],double h_activations[MAX_HIDDEN], double activations[OUTPUTS], double (*sigmoid)(double input), int label, double weights_ih[INPUTS][MAX_HIDDEN], double weights_ho[MAX_HIDDEN][OUTPUTS], int units)
{
  ...
  double target[OUTPUTS];
  double old_weights[MAX_HIDDEN][OUTPUTS];
  // compute target
  for (int i=0;i<OUTPUTS;i++){
    if (i == label) {
      target[i] = sigmoid(999);
  ...
  for (int j=0;j<OUTPUTS;j++){
    if (sigmoid == logistic){
      double final_error = target[j] - activations[j];
      error = (activations[j] * (1- activations[j])) * final_error;
    } else {
      double final_error = target[j] - activations[j];
      error = (1- activations[j] * activations[j]) * final_error;
    }
    errors[j] = error;
    for (int l=0;l<units;l++) {
      old_weights[l][j] = weights_ho[l][j];
      weights_ho[l][j] += ALPHA * error * h_activations[l];
    }
  }
  for (int i=0;i<units;i++) {
    total_error = 0.0;
    for (int j=0;j<OUTPUTS;j++) {
      total_error += old_weights[i][j] * errors[j];
    }
    if (sigmoid == logistic){
      ih_der = (h_activations[i] * (1- h_activations[i]));
    } else{
      ih_der = (1- h_activations[i] * h_activations[i]);
    }
    for (int k=0;k<INPUTS;k++) {
      weights_ih[k][i] += ALPHA * sample[k]* ih_der * total_error;
    }
  }
}
...
double logistic(double input)
{
 ...
 // TO DO: Implement this function!
 return 1/(exp(-input) + 1);		// <--- Should return the value of the logistic function on the input 
}
